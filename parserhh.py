...
def set_search_options(search_text, only_with_salary, region, responses_count, page=None, per_page=100):
    """
    ...
    :param responses_count: True or False Кол-во откликов : bool
    ...
    params = {
        ...
        'area': f'{region}',
        'responses_count_enabled': responses_count,
        'page': page,
        'per_page': per_page
    }
    return params
...
df.to_excel(f'{search_request}.xlsx', sheet_name=f'{search_request}')
...
print('=' * 60)
print(f'Общее время выполнения парсинга вакансии {search_request}: {round(((time.time() - curr_time) / 60), 2)} минут')
print(f'Файл логов "log.txt" {"создан" if os.path.exists("log.txt") else "не создан"}')
print(f'Файл логов ошибок {ERROR_LOG} {"создан" if os.path.exists(ERROR_LOG) else "не создан"}')
print(
    f'Таблица вакансий {search_request}.xlsx {"создана" if os.path.exists(f"{search_request}.xlsx") else "не создана"}')
